...
public class DecompressorTest {
	...
	public void queryTest() throws Throwable {
		...
		try {
			...
			Assert.assertEquals(0f, decompressor.query(new GenomicPosition("21", 59999, COORD_TYPE.ONEBASED)).getInterpolatedCoverage());
			Assert.assertEquals(1f, decompressor.query(new GenomicPosition("21", 60000, COORD_TYPE.ONEBASED)).getInterpolatedCoverage());
	...
	public void compareWithIteratorTest() throws Throwable {
		...
		try {
			CompressedCoverageIterator it = decompressor.getCoverageIterator();
			while (it.hasNext()) {
				Float hitIterator = it.next();
				GenomicPosition posIterator = it.getGenomicPosition();

				CoverageHit hitQuery = decompressor.query(posIterator);

				// System.out.println(posIterator.toString1basedOrig() + " ->  "
				// +
				// hitIterator.getInterpolatedCoverage() + "/" +
				// hitQuery.getInterpolatedCoverage());

				Assert.assertEquals(hitQuery.getInterpolatedCoverage(), hitIterator);
	...
	public void toBedTest() throws Throwable {
		...
		try {
			...
			while (it.hasNext()) {
				...
				// System.out.println(it.getGenomicPosition().toString1based() +
				// "/"
				// + hitIterator.getRoundedCoverage());
	...
	public void scalingTest() throws Throwable {
		...
		try {
			...
			while (it1.hasNext()) {
				...
				Assert.assertEquals(hit1 * scale, hit2);
				Assert.assertEquals(hit2, hit1 * scale);
				// Assert.assertEquals(hit1.getLowerBoundary() * scale,
				// hit2.getLowerBoundary());
				// Assert.assertEquals(hit1.getUpperBoundary() * scale,
				// hit2.getUpperBoundary());
				// Assert.assertEquals(hit1.getLowerBoundary(),
				// hit2.getLowerBoundaryRaw());
				// Assert.assertEquals(hit1.getUpperBoundary(),
				// hit2.getUpperBoundaryRaw());
			}
