...
public class CoverageTools {
	 ...
	 * @throws Throwable
	 */
	public static void extractSample(File cov1File, File cov1VcfFile, File sampleRegionsBed, String startPos, int N, int step, String format, PrintStream out)
			throws Throwable {
		if (debug)
			System.out.println("Load " + cov1File);
		CoverageDecompressor cov1 = CoverageDecompressor.loadFromFile(cov1File, cov1VcfFile);
		try {
			...
			if (startPos.equalsIgnoreCase("random")) {
				...
				start = sampleRegions.getRandomPosition();
			} else
			...
			while (it1.hasNext()) {
				...
				if (inRegion) {
					if (pos.get1Position() % step == 0) {
						...
						if (isWig)
							wout.push(pos, hit1.floatValue(), step);
						else
	...
	public static void main(String[] args) throws Throwable {
		...
		try {
			...
			if (subcommand.equalsIgnoreCase("annotateVCF")) {
				...
				opt = new Option("format", true, "Output format (raw | wig). Default: raw");
				opt.setRequired(false);
				options.addOption(opt);
				...
				extractSample(covFile, vcfFile, bedFile, start, N, step, format, out);
