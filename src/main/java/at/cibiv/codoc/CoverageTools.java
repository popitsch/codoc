...
public class CoverageTools {
	...
	public static void coverageToTsvColumn(File cov1File, int n, int win,
		...
		PrintWriter out = new PrintWriter(outF + ".temp");
		...
		try {
			...
			TabIterator ti = null;
			String[] t = null;
			GenomicPosition idx = null;
			if (outF.exists()) {
				ti = new TabIterator(outF, null);
				do {
					t = ti.next();
					if (t[0].equals("Pos"))
						break;
			...
			int c = 0, wc = 0;
			while (it1.hasNext()) {
				Integer cov = it1.nextCoverage();
				GenomicPosition pos = it1.getGenomicPosition();
				if (idx != null) {
					// outfile exists, check index position
					int cmp = pos.compareTo(idx);
					if (cmp > 0)
						cov = null;
					if (cmp >= 0) {
	...
	/**
	 * Extract a sample from a codoc file. FIXME N is not used!
	 * 
	 * @param covTFile
	 * @throws Throwable
	 */
...
//				"/Volumes/Temp/Niko/data.tsv", "-v" };
		...
		//
		// "-cov", "src/test/resources/covcompress/small.bam.codoc", "-start",
		// "first", "-N", "100", "-step", "5", "-o",
		...
		try {
			...
			if (subcommand.equalsIgnoreCase("stats")) {
				...
				opt = new Option("minScoreOut", true,
						"Minimum avg. score for writing an interval to bedOut (default: null).");
				opt.setRequired(false);
				options.addOption(opt);

				opt = new Option("maxScoreOut", true,
						"Minimum avg. score for writing an interval to bedOut (default: null).");
				opt.setRequired(false);
				options.addOption(opt);
				...
				opt = new Option("normalAlso", true,
						"Output of the variants that were found also in the normal (optional).");
				opt.setRequired(false);
				options.addOption(opt);

				opt = new Option("lowCoverage", true,
						"Output of the variants that had too low coverage in the normal (optional).");
				opt.setRequired(false);
				options.addOption(opt);
				...
				opt = new Option("win", true, "Window size (default: 100).");
				opt.setRequired(false);
				options.addOption(opt);
