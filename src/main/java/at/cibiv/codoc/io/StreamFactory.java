...
public class StreamFactory {
	...
	private static STREAM_DATA_TYPE sdtFromString(String s) throws CodocException {
		if (s == null)
			throw new CodocException("Invalid stream datatype " + s + "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]");
	...
	private static STREAM_LENGTH_TYPE sltFromString(String s) throws CodocException {
		if (s == null)
			throw new CodocException("Invalid stream lengthtype " + s + "  [" + Arrays.toString(STREAM_LENGTH_TYPE.values()) + "]");
	 ...
	 * @throws Throwable
	 */
	public static PushableStream<? extends Object> createTripleOutputStream(OutputStream o1, OutputStream o2, OutputStream o3, PropertyConfiguration conf,
			String prefix) throws CodocException {
		try {
			...
			// get stream type
			STREAM_TYPE type = stFromString(conf.getProperty(prefix + "-type",
					new CodocException("No stream type configured for " + prefix + "  [" + Arrays.toString(STREAM_TYPE.values()) + "]")));

			switch (type) {
			case PREFIX:
				STREAM_DATA_TYPE datatype3 = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for "
						+ prefix + "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]")));
				STREAM_LENGTH_TYPE lengthtype3 = sltFromString(conf.getProperty(prefix + "-lengthtype", new CodocException(
						"No stream lengthtype configured for " + prefix + "  [" + Arrays.toString(STREAM_LENGTH_TYPE.values()) + "]")));
	 ...
	 * @throws Throwable
	 */
	public static PushableStream<? extends Object> createOutputStream(OutputStream out, PropertyConfiguration conf, String prefix) throws Throwable {
		...
		case SUBBYTE:
			...
			try {
				String tmp = conf.getProperty(prefix + "-maxSymbols", new CodocException("No maxSymbols parameter configured for " + prefix));
		...
		case GOLOMB:
			STREAM_DATA_TYPE datatype4 = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for " + prefix
					+ "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]")));
			Integer k = null;
			try {
				String kstr = conf.getProperty(prefix + "-k", new CodocException("No golomb k parameter configured for " + prefix));
		...
		case RLE:
			STREAM_DATA_TYPE datatype = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for " + prefix
					+ "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]")));
			STREAM_LENGTH_TYPE lengthtype = sltFromString(conf.getProperty(prefix + "-lengthtype", new CodocException("No stream lengthtype configured for "
					+ prefix + "  [" + Arrays.toString(STREAM_LENGTH_TYPE.values()) + "]")));
			switch (datatype) {
			case BYTE:
				switch (lengthtype) {
				case BYTE:
			...
			case CHARACTER:
				...
				case CHARACTER:
					return new RunLengthOutputStream<Character, Character>(out, out, Character.class, Character.class, debug);
				case INTEGER:
					return new RunLengthOutputStream<Character, Integer>(out, out, Character.class, Integer.class, debug);
				}
			case INTEGER:
				switch (lengthtype) {
				case BYTE:
				...
				case CHARACTER:
					return new RunLengthOutputStream<Integer, Character>(out, out, Integer.class, Character.class, debug);
				case INTEGER:
		...
		case STANDARD:
			STREAM_DATA_TYPE datatype2 = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for " + prefix
					+ "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]")));
			switch (datatype2) {
			case BYTE:
	 ...
	 * @throws Throwable
	 */
	public static PushableStream<? extends Object> createHeterogenousOutputStream(OutputStream outData, OutputStream outLen, PropertyConfiguration conf,
		...
		case RLE:
			...
			case BYTE:
				...
				case CHARACTER:
					return new RunLengthOutputStream<Byte, Character>(outData, outLen, Byte.class, Character.class, debug);
				case INTEGER:
			...
			case CHARACTER:
				switch (lengthtype) {
				case BYTE:
					return new RunLengthOutputStream<Character, Byte>(outData, outLen, Character.class, Byte.class, debug);
				case CHARACTER:
					return new RunLengthOutputStream<Character, Character>(outData, outLen, Character.class, Character.class, debug);
				case INTEGER:
					return new RunLengthOutputStream<Character, Integer>(outData, outLen, Character.class, Integer.class, debug);
				}
			case INTEGER:
				switch (lengthtype) {
				case BYTE:
				...
				case CHARACTER:
					return new RunLengthOutputStream<Integer, Character>(outData, outLen, Integer.class, Character.class, debug);
				case INTEGER:
					return new RunLengthOutputStream<Integer, Integer>(outData, outLen, Integer.class, Integer.class, debug);
				}
			case STRING:
				switch (lengthtype) {
				case BYTE:
				...
				case CHARACTER:
					return new RunLengthOutputStream<String, Character>(outData, outLen, String.class, Character.class, debug);
				case INTEGER:
					return new RunLengthOutputStream<String, Integer>(outData, outLen, String.class, Integer.class, debug);
				}
			case SYMBOLS:
				switch (lengthtype) {
				case BYTE:
					return new RunLengthOutputStream<SYMBOLS, Byte>(new SymbolOutputStream(outData), outLen, SYMBOLS.class, Byte.class, debug);
				case CHARACTER:
					return new RunLengthOutputStream<SYMBOLS, Character>(new SymbolOutputStream(outData), outLen, SYMBOLS.class, Character.class, debug);
				case INTEGER:
					return new RunLengthOutputStream<SYMBOLS, Integer>(new SymbolOutputStream(outData), outLen, SYMBOLS.class, Integer.class, debug);
				}
			}
		case NULL:
			return new StandardOutputStream<Object>(null, Object.class, debug);
	...
	public static PopableStream<?> createTripleInputStream(InputStream in1, InputStream in2, InputStream in3, PropertyConfiguration conf, String prefix)
			throws CodocException {
		try {
			if ((conf == null) || (prefix == null))
				throw new CodocException("No proper stream configuration!");
	 ...
	 * @throws Throwable
	 */
	public static PopableStream<?> createInputStream(InputStream in, PropertyConfiguration conf, String prefix) throws IOException, CodocException {
		try {
			if ((conf == null) || (prefix == null))
				throw new CodocException("No proper stream configuration!");
			STREAM_TYPE type = stFromString(conf.getProperty(prefix + "-type",
			...
			case GOLOMB:
				STREAM_DATA_TYPE datatype4 = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for "
						+ prefix + "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]")));
				Integer k = null;
				try {
					String kstr = conf.getProperty(prefix + "-k", new CodocException("No golomb k parameter configured for " + prefix));
			...
			case RLE:
				STREAM_DATA_TYPE datatype = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for "
						+ prefix + "  [" + Arrays.toString(STREAM_DATA_TYPE.values()) + "]")));
				STREAM_LENGTH_TYPE lengthtype = sltFromString(conf.getProperty(prefix + "-lengthtype", new CodocException(
						"No stream lengthtype configured for " + prefix + "  [" + Arrays.toString(STREAM_LENGTH_TYPE.values()) + "]")));
				switch (datatype) {
				case BYTE:
				...
				case CHARACTER:
					...
					case CHARACTER:
						return new RunLengthInputStream<Character, Character>(in, in, Character.class, Character.class, debug);
					case INTEGER:
			...
			case STANDARD:
				STREAM_DATA_TYPE datatype2 = sdtFromString(conf.getProperty(prefix + "-datatype", new CodocException("No stream datatype configured for "
						+ prefix + "  [" + STREAM_DATA_TYPE.values() + "]")));
				switch (datatype2) {
				case BYTE:
	 ...
	 * @throws Throwable
	 */
	public static PopableStream<?> createHeterogenousInputStream(InputStream inData, InputStream inLen, PropertyConfiguration conf, String prefix)
			throws CodocException {
		try {
			if ((conf == null) || (prefix == null))
				throw new CodocException("No proper stream configuration!");
			...
			case RLE:
				...
				case CHARACTER:
					...
					case CHARACTER:
						return new RunLengthInputStream<Character, Character>(inData, inLen, Character.class, Character.class, debug);
					case INTEGER:
						return new RunLengthInputStream<Character, Integer>(inData, inLen, Character.class, Integer.class, debug);
					}
				case INTEGER:
					...
					case CHARACTER:
						return new RunLengthInputStream<Integer, Character>(inData, inLen, Integer.class, Character.class, debug);
					case INTEGER:
						return new RunLengthInputStream<Integer, Integer>(inData, inLen, Integer.class, Integer.class, debug);
					}
				case STRING:
					...
					case CHARACTER:
						return new RunLengthInputStream<String, Character>(inData, inLen, String.class, Character.class, debug);
					case INTEGER:
				...
				case SYMBOLS:
					...
					switch (lengthtype) {
					case BYTE:
						return new RunLengthInputStream<SYMBOLS, Byte>(new SymbolInputStream(inData), inLen, SYMBOLS.class, Byte.class, debug);
					case CHARACTER:
						return new RunLengthInputStream<SYMBOLS, Character>(new SymbolInputStream(inData), inLen, SYMBOLS.class, Character.class, debug);
					case INTEGER:
						return new RunLengthInputStream<SYMBOLS, Integer>(new SymbolInputStream(inData), inLen, SYMBOLS.class, Integer.class, debug);
					}
				}
			case NULL:
				return new StandardInputStream<Object>(null, Object.class, debug);
